...
class GVA_Layout_Frontend_Hook extends GVA_Layout_Model{
   ...
   function __construct(){
      add_action('orgarium/layouts/page', array($this, 'layout_page'));
      add_action('orgarium/layouts/single/post', array($this, 'layout_single_post'));
      add_action('orgarium/layouts/archive/post', array($this, 'layout_archive_post'));

      add_action('orgarium/layouts/single/template', array($this, 'single_template'));

      add_action('orgarium/layouts/single/product', array($this, 'layout_single_product'));
      add_action('orgarium/layouts/archive/product', array($this, 'layout_archive_product'));

      add_action('wp_enqueue_scripts', array($this, 'orgarium_themer_elementor_load_css_header'), 500);

   }

   public function orgarium_themer_elementor_load_css_header(){
      ...
      if($post && $post->post_type == 'page'){
         ...
         $post_meta_template = get_post_meta($post->ID, 'orgarium_template_layout', true);
   ...
   public function layout_page(){
      ...
      if($post_meta_template == '_without_layout' || empty($post_meta_template) || !metadata_exists('post', $post_id, 'orgarium_template_layout')){
   ...
   public function layout_single_product(){
      ...
      $post_id = $post->ID;
      $template_id = $this->get_template_default('single_product_layout');
      $post_template_id = get_post_meta($post_id, 'orgarium_template_layout', true);
