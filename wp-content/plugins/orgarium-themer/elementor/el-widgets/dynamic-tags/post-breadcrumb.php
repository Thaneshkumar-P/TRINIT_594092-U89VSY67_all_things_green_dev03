...
class GVAElement_Post_Breadcrumb extends GVAElement_Base{
	...
	public function get_title(){
		return esc_html__('Breadcrumb', 'orgarium-themer');
	}
	...
	protected function register_controls(){
		...
		$this->add_control(
			'bg_overlay_color',
			[
				'label' => esc_html__( 'Background Overlay Color', 'orgarium-themer' ),
		...
		$this->add_control(
	  	...
	  	$this->add_responsive_control(
			'padding_top',
			[
				'label' => __( 'Breadcrumb Padding Top', 'orgarium-themer' ),
		...
		$this->add_responsive_control(
			'padding_bottom',
			[
				'label' => __( 'Breadcrumb Padding Bottom', 'orgarium-themer' ),
				'type' => Controls_Manager::SLIDER,
				'default' => [
					'size' => 120,
				],
		...
		$this->add_control(
			'heading_content',
			[
				'label'     => __('Content & Style ------', 'orgarium-themer'),
				'type'      => Controls_Manager::HEADING,
			]
	  	);
		$this->add_control(
			'show_title',
			[
				'label'     => __('Show Titlte', 'orgarium-themer'),
		...
		$this->add_control(
			'title_color',
			[
				'label' => esc_html__( 'Title Color', 'orgarium-themer' ),
				'type' => Controls_Manager::COLOR,
				'selectors' => [
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .orgarium-post-tags .title',
			]
		);
		...
		$this->add_control(
			'show_links',
			[
				'label'     => __('Show Breadcrumb Links', 'orgarium-themer'),
				'type'      => Controls_Manager::SWITCHER,
				'default'   => 'yes',
	  	...
	  	$this->add_control(
			'breadcrumb_link_bg',
			[
				'label' => esc_html__( 'Background Color Breadcrumb Links', 'orgarium-themer' ),
				'type' => Controls_Manager::COLOR,
				'selectors' => [
	...
	public function breadcrumbs(){
	 	$delimiter = '';
	 	$home = esc_html__('Home', 'orgarium-themer');
	 	...
	 	if(!is_home() && !is_front_page() || is_paged()) {
			...
			if(is_category()){
			  	...
			  	$breadcrumb .= $before . esc_html__('Search results for', 'orgarium-themer') . '"' . get_search_query() . '"' . $after;
			  ...
			  $breadcrumb .= $before . esc_html__('Posts tagged', 'orgarium-themer') . '"' . single_tag_title('', false) . '"' . $after;
			  	...
			  	if($userdata){
				 	$breadcrumb .= $before . esc_html__('Articles posted by', 'orgarium-themer') . $userdata->display_name . $after;
			  	} 

			}elseif(is_404()){
			  	$breadcrumb .= $before . esc_html__('Error 404', 'orgarium-themer') . $after;
			}

			if(get_query_var('paged')){
			  	$breadcrumb .= $before . esc_html__('Page','orgarium-themer') . ' ' . get_query_var('paged') . $after;
			}
