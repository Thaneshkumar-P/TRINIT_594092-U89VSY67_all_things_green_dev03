<?php
  use Elementor\Icons_Manager;
   ...
   <?php if($style == 'style-2' || $style == 'style-3'){ ?>
      <?php 
         $has_icon_image = ($settings['use_icon_image'] == 'yes' && $settings['image_icon']['url']) ? true : false;
         $has_icon = ($settings['use_icon_image'] != 'yes' &&  !empty( $settings['selected_icon']['value'])) ? true : false;
      ?>
      <div <?php echo $this->get_render_attribute_string( 'block' ) ?>>
         <div class="icon-box-content">
           
            <?php if ( $has_icon_image || $has_icon ){ ?>
               <div class="icon-inner">
                  <span class="box-icon">
                     <?php 
                        if($has_icon){ 
                        ...
                        if($has_icon_image){ 
                           echo '<img src="' . $settings['image_icon']['url'] . '" alt="' . esc_attr($settings['title_text']) . '" />';
                        } 
                     ?> 
                  </span>
               </div>
